...
public final class MultiProducerSequencer extends AbstractSequencer
{
    ...
    public long next(int n) {
        ...
        long next;
        do {
            current = cursor.get(); // 原子读生产者序号器的当前位置
            next = current + n;   // 要发布到的目标序号

            long wrapPoint = next - bufferSize; // 环绕
            long cachedGatingSequence = gatingSequenceCache.get(); // 最慢的消费者已消费的最后一个事件的序号
            ...
            if (wrapPoint > cachedGatingSequence || cachedGatingSequence > current) {
                ...
                gatingSequenceCache.set(gatingSequence);    // 进行缓存

            } else if (cursor.compareAndSet(current, next)) {
                // 由于这个类会被多个线程访问，需要确保从  current = cursor.get();  到当前位置，cursor的值没有被其他线程改变，
                // 所以这里不能简单地用原子写更新 cursor的值。
                // 用CMS操作替换加锁，可以提升性能
                break;
            }
        } while (true);
     ...
     * division) -- Beause we can't wrap without the gating sequences moving
