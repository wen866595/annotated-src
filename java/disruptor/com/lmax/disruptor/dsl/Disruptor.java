...
{
    private final RingBuffer<T> ringBuffer ; // 核心，绝大多数功能都委托给ringBuffer处理
    private final Executor executor ;        // 事件处理逻辑的执行器
    private final ConsumerRepository<T> consumerRepository = new ConsumerRepository<T>();   // 事件处理器仓库
    ...
    public EventHandlerGroup<T> handleEventsWith(final EventProcessor... processors) {
        // 简单地把processor添加到消费者仓库
        for (EventProcessor processor : processors) {
            consumerRepository.add(processor);
        }
        // Util.getSequencesFor 返回每个EventProcessor对应的Sequence组成的数组。
    ...
    public RingBuffer<T> start() {
        // 获取当前消费者仓库里所有的事件处理器链的最后的Sequence，因为生产者判断一个槽位是否已被消费是根据消费者链的最后一个处理器所处的槽位来判断的
        Sequence[] gatingSequences = consumerRepository.getLastSequenceInChain(true);
        ringBuffer.addGatingSequences(gatingSequences); // （原子性地）设置消费者关卡

        checkOnlyStartedOnce(); // 确保容器只启动一次
        for (ConsumerInfo consumerInfo : consumerRepository) {  // 逐个启动事件处理器
