...
public final class WorkerPool<T>
{
    ...
    public WorkerPool(final RingBuffer<T> ringBuffer, final SequenceBarrier sequenceBarrier,
                      final ExceptionHandler exceptionHandler, final WorkHandler<T>... workHandlers)
    {
        ...
        {
            workProcessors[i] = new WorkProcessor<T>(ringBuffer, sequenceBarrier, workHandlers[i], exceptionHandler,
                                                     workSequence);
        }
    }
     ...
     *            to distribute the work load across.
     */
    public WorkerPool(final EventFactory<T> eventFactory, final ExceptionHandler exceptionHandler,
                      final WorkHandler<T>... workHandlers)
    {
        ...
        for (int i = 0; i < numWorkers; i++)
        {
            workProcessors[i] = new WorkProcessor<T>(ringBuffer, barrier, workHandlers[i], exceptionHandler,
                                                     workSequence);
        }
