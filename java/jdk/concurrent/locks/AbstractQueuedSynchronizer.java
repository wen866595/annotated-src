...
public abstract class AbstractQueuedSynchronizer
    ...
    static final class Node {
        ...
        /*
         * 状态字段，只能取下面的值：
         * SIGNAL(-1)：         这个结点的后继是（或很快是）阻塞的（通过park），所以当前结点必须unpark它的后继，当它释放或取消时。
         *                为了避免竞争，acquire方法必须首先表明它们需要一个信号，然后再次尝试原子性acquire，如果失败了就阻塞。
         *                
         ...
         *                
         * PROPAGATE(-3)：releaseShared应该传递给其他结点。这是在doReleaseShared里设置（仅仅是头结点）以确保传递继续，即使
         *                其他操作有干涉。
         * 
         * 0：                                 非以上任何值。
         * 
         ...
         * 根据前驱结点的取消，我们使查找一个非取消结点的while循环短路，
         * 这个总是会退出，因为头结点从不会是取消了的：一个结点成为头只能是一次成功的acquire操作结果。
         * 
         ...
         * 所以看到next字段为空不一定表示它就是队列的尾结点。然而，如果next字段看起来是空，我们可以从tail向前遍历进行双重检查。
        ...
        for (;;) {
            ...
            if (t == null) { // 队列是空，必须初始化。
                ...
                if (compareAndSetTail(t, node)) {
                    t.next = node; // 在把新结点设置为tail后才能更新前驱的next字段，这样，即使CAS失败了也不会影响原来的连接关系。
                    return t;
                }
            }
        }
	...
	private Node addWaiter(Node mode) {
	    ...
	    Node pred = tail;
	    if (pred != null) {
	    	// 把新结点的前驱指向pred，必须在下面的CAS完成之前设置，这样确保一旦CAS成功后，从tail向后遍历是ok的。
	        node.prev = pred;
    ...
    private void cancelAcquire(Node node) {
        ...
        if (node == tail && compareAndSetTail(node, pred)) {
            ...
            compareAndSetNext(pred, predNext, null);// 如果失败了，说明有其他线程的取消/信号操作移除了predNext
        } else {
            ...
            if (pred != head &&
                ...
                pred.thread != null) {
            	// 这个if条件成立，说明node有一个需要signal的前驱，让这个前驱指向node的后继，这样传播的链是完整的。
                Node next = node.next;
    ...
    private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
        ...
        if (ws > 0) {
            ...
            // 把找到的结点指向node，那么当前pred与node之间的已取消结点就不再被引用了，可以被垃圾回收。
            pred.next = node;
        } else {
	    ...
	    try {
	        ...
	        for (;;) { // 死循环，用于acquire失败后重试
	            ...
	            if (shouldParkAfterFailedAcquire(p, node) &&
	                ...
	                interrupted = true;	// 只有线程被中断过一次就会记录下来
	        }
	    } finally {
	        if (failed)
	        	// acquire失败（比如因为中断），取消acquire
	            cancelAcquire(node);
	    }
	}
	...
	public final void acquire(int arg) {
	    if (!tryAcquire(arg) && // 首先尝试获取
	    	...
	    	// 失败后加入等待队列，再从队列里再次尝试获取；成功获取后才返回，返回的boolean表示线程是否曾经被中断。
     ...
     * first invoking at least once {@link #try},
     * returning on success.  Otherwise the thread is queued, possibly
     * repeatedly blocking and unblocking, invoking {@link
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
         ...
         */
        // 把first开始的第一个结点添加到sync队列，只唤醒一个
        private void doSignal(Node first) {
            do {
                if ( (firstWaiter = first.nextWaiter) == null)
                    lastWaiter = null;
                first.nextWaiter = null;
            		// transferForSignal 把结点first添加到等待获取的队列（sync队列），成功返回true，否则false
        ...
        public final void await() throws InterruptedException {
            ...
            // 释放锁之后其他的唤醒线程可能会把它加入同步队列
            while (!isOnSyncQueue(node)) { // 如果结点不在同步获取等待队列上，则进入等待
                LockSupport.park(this);
